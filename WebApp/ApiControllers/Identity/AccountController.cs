...
public class AccountController : ControllerBase
{
    ...
    )
    {
        ...
        if (appUser == null)
        {
            ...
            await Task.Delay(TimeSpan.FromMilliseconds(new Random().Next(35, 150)));
            return NotFound("User/Password problem");
        }

        // Verify Password
        var res = await _signInManager
            .CheckPasswordSignInAsync(appUser, loginInfo.Password, false);
        if (!res.Succeeded)
        {
            _logger.LogWarning("WebApi login failed: Password {} for email {} was wrong...",
                loginInfo.Password, loginInfo.Email);
            await Task.Delay(TimeSpan.FromMilliseconds(new Random().Next(50, 120)));
            return NotFound("User/Password problem");
        }

        // Tokens
        var deletedRows =  _context.RefreshTokens
        ...
        )
        {
            ...
            var isValid = IdentityHelpers.ValidateJwt(
            ...
            // Make new RefreshToken, Keep old one still valid for some time
